...
var controller = Ember.Controller.extend({
  ...
  taskd_trust_settings: [
    {
      short: "no",
      long: "Validate taskserver using an uploaded CA Certificate",
    },
    {
      short: "yes",
      long:
        "Trust taskserver implicitly; do not validate using a CA Certificate",
    },
  ],
  ...
  sync_with_init: function () {
    ...
    return this.ajaxRequest({
      ...
      function () {
        ...
        this.success_message("Initial sync completed successfully.");
      }.bind(this),
      function (msg) {
        this.error_message(
  ...
  submit_taskd: function (data, after) {
    if (data.certificate === false || data.key === false || data.ca === false) {
      self.error_message("An error was encountered while uploading your files");
      return;
    } else if (
      !data.certificate ||
      !data.key ||
      (!data.ca && data.trust === "no")
    ) {
      return;
    }
  ...
  actions: {
    ...
    trello_force_resynchronization: function () {
      var url = this.get("applicationController").urls
        .trello_resynchronization_url;
      return this.ajaxRequest({
        url: url,
        type: "POST",
        data: {},
