...
var controller = Ember.Controller.extend({
  ...
  checkStatusUpdater: function () {
    ...
    if (
      ...
      if (!since) {
        ...
        log.pushObject([now, "Connection appears to be disconnected"]);
        this.set("statusUpdaterErrorred", true);
  ...
  startEventStream: function () {
    ...
    log.pushObject([new Date(), "Starting with HEAD " + head]);
    var statusUpdater = this.get("statusUpdater");
    if (
  ...
  actions: {
    ...
    _help_hidden: function () {
      ...
      if (originalBgColor) {
        $("nav").css("background-color", originalBgColor);
      }
    ...
    show_help: function () {
      ...
      if ($(".chardinjs-overlay").length) {
        ...
        if (currentBgColor !== targetBgColor) {
          this.set("_chardin_original_bgcolor", currentBgColor);
        }
        $("nav").css("background-color", targetBgColor);

        // Show corner help information
        var helpInstructionsView = Ember.View.create({
          template: this.container.lookup("template:helpInstructions"),
        });
        helpInstructionsView.appendTo($(".chardinjs-overlay"));
      }
    },
    show_log: function () {
