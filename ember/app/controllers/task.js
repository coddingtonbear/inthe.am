...
var controller = ObjectController.extend({
  ...
  actions: {
    complete: function () {
      var result = confirm(
        "Are you sure you would like to mark this task as completed?"
      );
      if (result) {
        var self = this;
    ...
    delete_annotation: function (rawDescription) {
      ...
      model.save().then(
        ...
        function (reason) {
          ...
          self
            .get("applicationController")
            .error_message("Could not delete annotation!");
        }
      );
    },
    start: function () {
      var model = this.get("model");
      var url =
        this.store.adapterFor("task").buildURL("task", model.get("uuid")) +
        "start/";
    ...
    delete: function () {
      ...
      if (result) {
        var model = this.get("model");
        var url =
          this.store.adapterFor("task").buildURL("task", this.get("uuid")) +
          "delete/";
        this.get("applicationController").showLoading();
