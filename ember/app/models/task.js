...
var model = DS.Model.extend({
  ...
  taskwarrior_class: function () {
    ...
    if (this.get("start")) {
      ...
      value = "blocked";
    } else if (this.get("due") && moment(this.get("due")).isBefore(moment())) {
      value = "overdue";
    } else if (
      this.get("due") &&
      ...
      moment(moment().format("YYYYMMDD"), "YYYYMMDD")
        .add(7, "days")
        .isAfter(this.get("due"))
    ) {
      value = "due";
  ...
  ticketIdsToObjects: function (value) {
    ...
    if (value) {
      for (var i = 0; i < value.length; i++) {
        promises.pushObject(this.store.findRecord("task", value[i]));
      }
    }
    return DS.PromiseArray.create({
      promise: Ember.RSVP.Promise.all(
        promises,
        "TicketIDs " + value + " to Objects"
      ),
    });
  },
