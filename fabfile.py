...
def deploy(
    ...
    pubsub_message = {
        "data": {"should_refresh": True if refresh == "yes" else False},
        "system": True,
    }
    ...
    run("redis-cli -n 1 PUBLISH __general__ '%s'" % json.dumps(pre_message))
    if chown == "yes":
    ...
    run("redis-cli -n 1 PUBLISH __general__ '%s'" % json.dumps(post_message))
    if restart == "yes":
