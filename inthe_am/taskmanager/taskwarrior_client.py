...
class TaskwarriorError(Exception):
    def __init__(self, stderr, stdout, code):
        ...
        self.code = code
        message = f"{self.code}: {self.stdout} :: {self.stderr}"
...
class TaskwarriorClient(TaskWarriorShellout):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, marshal=True, **kwargs)
    ...
    def _get_acceptable_properties(self):
        return list(TaskwTask.FIELDS.keys()) + list(self.config.get_udas().keys())
    ...
    def _strip_unsafe_args(self, *args):
        ...
        for raw_arg in args[1:]:
            ...
            if ":" in arg:
                ...
                if acceptable_cmd:
                    final_args.append(":".join([acceptable_cmd, value]))
                else:
    ...
    def _strip_unsafe_chars(self, incoming):
        return "".join(char for char in incoming if curses.ascii.isprint(char))

    def _execute_safe(self, *args):
        return self._execute(*self._strip_unsafe_args(*args))
    ...
    def _get_logger(self, cmd):
        try:
            ...
            command = str(cmd[0])
        return logging.getLogger(f"{__name__}.{command}")
    ...
    def gc(self):
        ...
        try:
            self._execute("list", config_overrides=overrides)
    ...
    def _execute(self, *args, **kwargs):
        ...
        self.send_client_message(
            ...
            "taskwarrior.execute",
            {
                "command": " ".join(pipes.quote(c.decode("utf-8")) for c in command),
                "arguments": " ".join(pipes.quote(str(arg)) for arg in args),
                "stderr": stderr,
                "stdout": stdout,
    ...
    def import_task(self, value):
        with tempfile.NamedTemporaryFile() as jsonout:
            jsonout.write(json.dumps(value, cls=OneWaySafeJSONEncoder))
            jsonout.flush()
    ...
    def _get_task_objects(self, obj, *args):
        results = super()._get_task_objects(obj, *args)
