def merge_task_data(alpha, beta):
    ...
    if unsalvageable:
        extra_data = "\n".join(
            ...
            [f"{k}: {v}  " for k, v in unsalvageable.items()]
        )
        message = message + "\nExtra fields:\n" + extra_data
...
def find_all_duplicate_tasks(store):
    ...
    all_tasks_by_imask = {}
    for task in tasks:
        all_tasks_by_imask.setdefault(task["parent"], {}).setdefault(
            task["imask"], set()
        ).add(task["uuid"])
...
def merge_all_duplicate_tasks(store, duplicates=None):
    ...
    for duplicate in duplicates:
        ...
        for task_id in duplicate:
            ...
            if (
                first_task is None
                or task["entry"] < first_task["entry"]
                or (
                    task["entry"] == first_task["entry"]
                    and task["id"] < first_task["id"]
                )
            ):
